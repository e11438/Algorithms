...
public class Graph {
	...
	private void buildGraphFromFile(String fileName) throws FileNotFoundException {
		try {
			...
			if (line != null) {
				...
				vertexarr = new String[vertexList.length];
	...
	//dijkstra method
		...
		public static neighbor returnMin(List<neighbor> n,String pre,ArrayList<String> v ){
			int min=Integer.MAX_VALUE;
			neighbor name=null;
			for(int i=0;i<n.size();i++){
				if(n.get(i).weight<min&&!n.get(i).vertex.equals(pre)&&v.contains(n.get(i).vertex)){
					min=n.get(i).weight;
					name=n.get(i);
				}
			}
